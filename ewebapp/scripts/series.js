...
async function getAllMovies(params) {
    ...
    const url = 'https://moviesdatabase.p.rapidapi.com/titles?titleType=tvSeries&info=base_info&startYear=2020&limit=16';
const options = {
	method: 'GET',
	headers: {
...
async function searchItem(val,btn){
    let listStorage = localStorage.getItem('bookmark');
    let listBkStg = listStorage? Array.from(JSON.parse(listStorage)):[] ;
    const url =  `https://moviesdatabase.p.rapidapi.com/titles/search/title/${val}?exact=false&titleType=tvSeries&info=base_info&sort=year.decr&limit=20`;
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': '4140faa556msh86a17e459830975p1b8b8djsn02f5450116c2',
